...
namespace OWO {
    const PLUGIN_NAMESPACE = 'OWOPlugin';

    spl_autoload_register(function (string $className) {
        $isDefinedAlias = defined('__OWO_CLASS_ALIASES__');
        $isNamespace = strpos($className, '\\') !== false;
        $isAlias = $isDefinedAlias && isset(__OWO_CLASS_ALIASES__[$className]);
        $isPlugin = false;
        ...
        if (strpos($className, '\\') !== false) {
            ...
            if ($isPlugin) {
                ...
                if ($isDefinedAlias) {
                    $alias = array_search('\\' . ltrim($className, '\\'), __OWO_CLASS_ALIASES__);
                }
            ...
            $isPlugin = !$isAlias && !preg_match("/^(OWO|Widget|IXR)_/", $className);

            if ($isPlugin) {
                $alias = '\\OWOPlugin\\' . str_replace('_', '\\', $className);
                $path = str_replace('_', '/', $className);
            } else {
                $alias = $isAlias ? __OWO_CLASS_ALIASES__[$className]
        ...
        // load class file
        $path .= '.php';
        $defaultFile = __OWO_ROOT_DIR__ . '/var/' . $path;

        if (file_exists($defaultFile) && !$isPlugin) {
            include_once $defaultFile;
        } else {
            $pluginFile = __OWO_ROOT_DIR__ . __OWO_PLUGIN_DIR__ . '/' . $path;
    ...
    class Common
    {
        ...
        public static function error(\Throwable $exception)
        {
            ...
            if ($exception instanceof \OWO\Db\Exception) {
                $code = 500;

                //覆盖原始错误信息
                $message = 'Database Server Error';

                if ($exception instanceof \OWO\Db\Adapter\ConnectionException) {
                    $code = 503;
                    $message = 'Error establishing a database connection';
                } elseif ($exception instanceof \OWO\Db\Adapter\SQLException) {
            ...
            if (defined('__OWO_EXCEPTION_FILE__')) {
                require_once __OWO_EXCEPTION_FILE__;
            } else {
                echo
                <<<EOF
<!DOCTYPE html>
...
EOF;
        ...
        public static function gravatarUrl(
            ...
            if (defined('__OWO_GRAVATAR_PREFIX__')) {
                $url = __OWO_GRAVATAR_PREFIX__;
            } else {
