...
class Admin extends Comments
{
    ...
    public function execute()
    {
        ...
        if (!$this->user->pass('editor', true)) {
            ...
            if ('on' == $this->request->__OWO_all_comments) {
                Cookie::set('__OWO_all_comments', 'on');
            } else {
                if ('off' == $this->request->__OWO_all_comments) {
                    Cookie::set('__OWO_all_comments', 'off');
                }

                if ('on' != Cookie::get('__OWO_all_comments')) {
                    $select->where('table.comments.ownerId = ?', $this->user->uid);
                }
            }
        }
