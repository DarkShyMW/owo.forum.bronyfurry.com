...
use OWO\Cookie;
use OWO\Router;
use OWO\Widget\Helper\PageNavigator\Box;
...
class Archive extends Comments
{
    ...
    public function execute()
    {
        ...
        $commentsAuthor = Cookie::get('__OWO_remember_author');
        $commentsMail = Cookie::get('__OWO_remember_mail');
        ...
        if ($this->options->commentsThreaded && !$this->isTopLevel && $this->parameter->allowComment) {
            ...
            if (!$plugged) {
                echo '<a href="' . substr($this->permalink, 0, - strlen($this->theId) - 1) . '?replyTo=' . $this->coid .
                    '#' . $this->parameter->respondId . '" rel="nofollow" onclick="return OWOComment.reply(\'' .
        ...
        if ($this->options->commentsThreaded) {
            ...
            if (!$plugged) {
                ...
                echo '<a id="cancel-comment-reply-link" href="' . $this->parameter->parentContent['permalink'] . '#' . $this->parameter->respondId .
                    '" rel="nofollow"' . ($replyId ? '' : ' style="display:none"') . ' onclick="return OWOComment.cancelReply();">' . $word . '</a>';
            }
        }
    }
