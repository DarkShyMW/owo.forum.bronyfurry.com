...
use OWO\Db;
use OWO\Router;
use OWO\Router\Parser;
use OWO\Widget;
use OWO\Plugin\Exception as PluginException;
...
class Options extends Base
{
    ...
    public function execute()
    {
        if (isset($this->db)) {
            ...
            // finish install
            if (empty($values)) {
                $this->response->redirect(defined('__OWO_ADMIN__')
        ...
        /** 动态获取根目录 */
        $this->rootUrl = defined('__OWO_ROOT_URL__') ? __OWO_ROOT_URL__ : $this->request->getRequestRoot();
        if (defined('__OWO_ADMIN__')) {
            /** 识别在admin目录中的情况 */
            $adminDir = '/' . trim(defined('__OWO_ADMIN_DIR__') ? __OWO_ADMIN_DIR__ : '/admin/', '/');
            $this->rootUrl = substr($this->rootUrl, 0, - strlen($adminDir));
        }

        /** 初始化站点信息 */
        if (defined('__OWO_SITE_URL__')) {
            $this->siteUrl = __OWO_SITE_URL__;
        } elseif (defined('__OWO_DYNAMIC_SITE_URL__') && __OWO_DYNAMIC_SITE_URL__) {
    ...
    {
        return __OWO_ROOT_DIR__ . __OWO_THEME_DIR__ . '/' . trim($theme, './') . '/' . trim($file, './');
    }
    ...
    public function themeUrl(?string $path = null, ?string $theme = null)
    {
        if (!isset($theme)) {
            ...
            $url = defined('__OWO_THEME_URL__') ? __OWO_THEME_URL__ :
                Common::url(__OWO_THEME_DIR__ . '/' . $theme, $this->siteUrl);
    ...
    {
        return ($this->rewrite || (defined('__OWO_REWRITE__') && __OWO_REWRITE__))
    ...
    {
        return defined('__OWO_PLUGIN_URL__') ? __OWO_PLUGIN_URL__ :
            Common::url(__OWO_PLUGIN_DIR__, $this->siteUrl);
    }
    ...
    {
        return Common::url(__OWO_PLUGIN_DIR__, __OWO_ROOT_DIR__);
    }
    ...
    {
        return Common::url(defined('__OWO_ADMIN_DIR__') ?
            __OWO_ADMIN_DIR__ : '/admin/', $this->rootUrl);
    }
