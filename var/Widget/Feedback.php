...
class Feedback extends Comments implements ActionInterface
{
    ...
    private function comment()
    {
        ...
        if (!$this->user->hasLogin()) {
            ...
            $expire = 30 * 24 * 3600;
            Cookie::set('__OWO_remember_author', $comment['author'], $expire);
            Cookie::set('__OWO_remember_mail', $comment['mail'], $expire);
            Cookie::set('__OWO_remember_url', $comment['url'], $expire);
        } else {
        ...
        if ($error = $validator->run($comment)) {
            /** 记录文字 */
            Cookie::set('__OWO_remember_text', $comment['text']);
        ...
        Cookie::delete('__OWO_remember_text');
     ...
     * 引用处理函数
     *
     * @throws Exception|\OWO\Db\Exception
     */
