...
{
    public const HEADER = '%OWO_BACKUP_XXXX%';
    ...
    {
        return array_map('basename', glob(__OWO_BACKUP_DIR__ . '/*.dat'));
    }
    ...
    private function import()
    {
        ...
        if (!empty($_FILES)) {
            ...
            $path = __OWO_BACKUP_DIR__ . '/' . $this->request->get('file');
    ...
    private function parseHeader($str, &$version): bool
    {
        ...
        if (!preg_match("/%OWO_BACKUP_[A-Z0-9]{4}%/", $str)) {
            return false;
        }
    ...
    private function reLogin(&$user)
    {
        ...
        Cookie::set('__OWO_uid', $user['uid']);
        Cookie::set('__OWO_authCode', Common::hash($user['authCode']));
        $this->login = true;
    }
}
