...
use OWO\Router;
use OWO\Widget;
use OWO\Widget\Exception as WidgetException;
...
class XmlRpc extends Contents implements ActionInterface, Hook
{
    ...
    public function wpGetPage(int $blogId, int $pageId, string $userName, string $password): array
    {
        ...
        return [
            ...
            'page_id'                => $page->cid,
            'page_status'            => $this->OWOToWordpressStatus($page->status, 'page'),
            'description'            => $excerpt,
            'title'                  => $page->title,
            'link'                   => $page->permalink,
    ...
    public function wpGetPages(int $blogId, string $userName, string $password): array
    {
        ...
        while ($pages->next()) {
            ...
            $pageStructs[] = [
                ...
                'page_status'            => $this->OWOToWordpressStatus(
    ...
    public function mwNewPost(int $blogId, string $userName, string $password, array $content, bool $publish): int
    {
        ...
        if (isset($content["{$type}_status"])) {
            $status = $this->wordpressToOWOStatus($content["{$type}_status"], $type);
    ...
    public function wpGetComment(int $blogId, string $userName, string $password, int $commentId): array
    {
        ...
        return [
            ...
            'status'           => $this->OWOToWordpressStatus($comment->status, 'comment'),
    ...
    public function wpGetComments(int $blogId, string $userName, string $password, array $struct): array
    {
        ...
        if (!empty($struct['status'])) {
            $input['status'] = $this->wordpressToOWOStatus($struct['status'], 'comment');
        } else {
            $input['__OWO_all_comments'] = 'on';
        }
        ...
        while ($comments->next()) {
            $commentsStruct[] = [
                ...
                'status'           => $this->OWOToWordpressStatus($comments->status, 'comment'),
    ...
    public function mwGetPost(int $postId, string $userName, string $password): array
    {
        ...
        return [
            ...
            'post_status'            => $this->OWOToWordpressStatus($post->status, 'post'),
            'custom_fields'          => [],
            'sticky'                 => 0
        ];
    }
    ...
    public function mwGetRecentPosts(int $blogId, string $userName, string $password, int $postsNum): array
    {
        ...
        while ($posts->next()) {
            ...
            $postStructs[] = [
                ...
                'post_status'            => $this->OWOToWordpressStatus(
...
<rsd version="1.0" xmlns="http://archipelago.phrasewise.com/rsd">
    <service>
        <engineName>OWO</engineName>
        <engineLink>http://www.OWO.org/</engineLink>
...
EOF;
    ...
    /**
     * 将OWO的状态类型转换为wordperss的风格
     *
     * @param string $status OWO的状态
     * @param string $type 内容类型
     * @return string
     */
    private function OWOToWordpressStatus(string $status, string $type = 'post'): string
    {
        if ('post' == $type) {
            /** 文章状态 */
            switch ($status) {
     ...
     * @return string
     */
    private function wordpressToOWOStatus(string $status, string $type = 'post'): string
    {
        if ('post' == $type) {
            /** 文章状态 */
            switch ($status) {
