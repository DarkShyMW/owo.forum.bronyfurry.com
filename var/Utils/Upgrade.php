...
class Upgrade
{
    ...
    {
        if (is_writeable(__OWO_ROOT_DIR__ . '/config.inc.php')) {
            $handle = fopen(__OWO_ROOT_DIR__ . '/config.inc.php', 'ab');
            fwrite($handle, '
/** 初始化时区 */
OWO_Date::setTimezoneOffset($options->timezone);
');
            fclose($handle);
        } else {
    ...
    {
        //创建上传目录
        $uploadDir = Common::url(Upload::UPLOAD_DIR, __OWO_ROOT_DIR__);
        if (is_dir($uploadDir)) {
            if (!is_writeable($uploadDir)) {
                if (!@chmod($uploadDir, 0755)) {
                    throw new \OWO\Widget\Exception(_t('上传目录无法写入, 请手动将安装目录下的 %s 目录的权限设置为可写然后继续升级', Upload::UPLOAD_DIR));
                }
            }
        } else {
            if (!@mkdir($uploadDir, 0755)) {
                throw new \OWO\Widget\Exception(_t('上传目录无法创建, 请手动创建安装目录下的 %s 目录, 并将它的权限设置为可写然后继续升级', Upload::UPLOAD_DIR));
            }
        }
    ...
    public static function v0_7r9_7_2($db, $options)
    {
        ...
        if (is_writeable(__OWO_ROOT_DIR__ . '/config.inc.php')) {
            $contents = file_get_contents(__OWO_ROOT_DIR__ . '/config.inc.php');
            $contents = preg_replace("/Common::init([^;]+);/is", "Common::init(array(
    'autoLoad'          =>  true,
    'exception'         =>  'Widget_ExceptionHandle',
    'gpc'               =>  true
));", $contents);
            $contents = preg_replace("/\s*(\/[^\/]+\/)?\s*OWO_Widget::widget([^;]+);/is", '', $contents);
            $contents = preg_replace("/\s*(\/[^\/]+\/)?\s*OWO_Router::setRoutes([^;]+);/is", '', $contents);
            $contents = preg_replace("/\s*(\/[^\/]+\/)?\s*OWO_Plugin::init([^;]+);/is", '', $contents);
            $contents = preg_replace("/\s*(\/[^\/]+\/)?\s*OWO_Date::setTimezoneOffset([^;]+);/is", '', $contents);
            file_put_contents(__OWO_ROOT_DIR__ . '/config.inc.php', $contents);

        } else {
            /** 升级提示 */
            return _t('建议您在升级到 OWO 0.7/9.7.2 以后的版本后, 立刻执行<a href="http://OWO.org/upgrade/9.7.2">以下优化步骤</a>');
        }
    }
