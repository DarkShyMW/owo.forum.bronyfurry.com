...
if (!file_exists(dirname(__FILE__) . '/config.inc.php')) {
    ...
    define('__OWO_PLUGIN_DIR__', '/usr/plugins');

    // theme directory (relative path)
    define('__OWO_THEME_DIR__', '/usr/themes');

    // admin directory (relative path)
    define('__OWO_ADMIN_DIR__', '/admin/');
...
function install_get_lang(): string
{
    ...
    if (!empty($serverLang)) {
        ...
        if ($request->is('lang')) {
            ...
            \OWO\Cookie::set('lang', $lang);
        }

        return \OWO\Cookie::get('lang', $lang);
    }
}
...
function install_get_db_drivers(): array
{
    ...
    if (\OWO\Db\Adapter\Pdo\Mysql::isAvailable()) {
        $drivers['Pdo_Mysql'] = _t('Pdo 驱动 Mysql 适配器');
    }

    if (\OWO\Db\Adapter\Pdo\SQLite::isAvailable()) {
        $drivers['Pdo_SQLite'] = _t('Pdo 驱动 SQLite 适配器');
    }

    if (\OWO\Db\Adapter\Pdo\Pgsql::isAvailable()) {
        $drivers['Pdo_Pgsql'] = _t('Pdo 驱动 PostgreSql 适配器');
    }

    if (\OWO\Db\Adapter\Mysqli::isAvailable()) {
        $drivers['Mysqli'] = _t('Mysql 原生函数适配器');
    }

    if (\OWO\Db\Adapter\SQLite::isAvailable()) {
        $drivers['SQLite'] = _t('SQLite 原生函数适配器');
    }

    if (\OWO\Db\Adapter\Pgsql::isAvailable()) {
...
// config db
\$db = new \OWO\Db('{$adapter}', '{$dbPrefix}');
\$db->addServer(" . (var_export($dbConfig, true)) . ", \OWO\Db::READ | \OWO\Db::WRITE);
\OWO\Db::set(\$db);
";
    ...
    if (!$return) {
        $configWritten = @file_put_contents(__OWO_ROOT_DIR__ . '/config.inc.php', $code) !== false;
    }
...
function install_remove_config_file()
{
    ...
    if ($configWritten) {
        unlink(__OWO_ROOT_DIR__ . '/config.inc.php');
    }
}
...
function install_check(string $type): bool
{
    switch ($type) {
        case 'config':
            return file_exists(__OWO_ROOT_DIR__ . '/config.inc.php');
        case 'db_structure':
        case 'db_data':
            global $installDb;
            ...
            try {
                ...
                return true;
            } catch (\OWO\Db\Adapter\SQLException $e) {
                return false;
            }
...
{
    \OWO\Response::getInstance()->setContentType('application/json')
...
{
    \OWO\Response::getInstance()->setStatus(302)
...
{
    ?>
    <div id="success" class="row OWO-page-main hidden">
                    ...
                    if (data.success) {
                        if (data.message) {
                            ...
                            if (data.config) {
                                ...
                                $('.OWO-page-main:first').addClass('hidden');
...
function install_step_1()
{
    ...
    $lang = install_get_lang();
    ?>
    <div class="row OWO-page-main">
        <div class="col-mb-12 col-tb-8 col-tb-offset-2">
            <div class="OWO-page-title">
                <h2><?php _e('欢迎使用 OWO'); ?></h2>
            </div>
            <div id="OWO-welcome">
                <form autocomplete="off" method="post" action="install.php">
                    ...
                    <h3><?php _e('许可及协议'); ?></h3>
                    <ul>
                        <li><?php _e('OWO 基于 <a href="http://www.gnu.org/copyleft/gpl.html">GPL</a> 协议发布, 我们允许用户在 GPL 协议许可的范围内使用, 拷贝, 修改和分发此程序.'); ?>
                            <?php _e('在GPL许可的范围内, 您可以自由地将其用于商业以及非商业用途.'); ?></li>
                        <li><?php _e('OWO 软件由其社区提供支持, 核心开发团队负责维护程序日常开发工作以及新特性的制定.'); ?>
    ...
    <div class="row OWO-page-main">
        <div class="col-mb-12 col-tb-8 col-tb-offset-2">
            ...
            <form autocomplete="off" action="install.php" method="post">
                <ul class="OWO-option">
                    <li>
                        <label for="dbAdapter" class="OWO-label"><?php _e('数据库适配器'); ?></label>
                ...
                <ul class="OWO-option">
                    <li>
                        <label class="OWO-label" for="dbPrefix"><?php _e('数据库前缀'); ?></label>
                        <input type="text" class="text" name="dbPrefix" id="dbPrefix" value="OWO_" />
                        <p class="description"><?php _e('默认前缀是 "OWO_"'); ?></p>
                    </li>
                </ul>
                <?php require_once './install/' . $type . '.php'; ?>


                <ul class="OWO-option OWO-option-submit">
                    <li>
...
function install_step_2_perform()
{
    ...
    $configMap = [
        ...
        'SQLite' => [
            'dbFile' => __OWO_ROOT_DIR__ . '/usr/' . uniqid() . '.db'
        ]
    ];

    if (install_is_cli()) {
        $config = [
            'dbHost' => $request->getServer('OWO_DB_HOST'),
            'dbUser' => $request->getServer('OWO_DB_USER'),
            'dbPassword' => $request->getServer('OWO_DB_PASSWORD'),
            'dbCharset' => $request->getServer('OWO_DB_CHARSET'),
            'dbPort' => $request->getServer('OWO_DB_PORT'),
            'dbDatabase' => $request->getServer('OWO_DB_DATABASE'),
            'dbFile' => $request->getServer('OWO_DB_FILE'),
            'dbDsn' => $request->getServer('OWO_DB_DSN'),
            'dbEngine' => $request->getServer('OWO_DB_ENGINE'),
            'dbPrefix' => $request->getServer('OWO_DB_PREFIX', 'OWO_'),
            'dbAdapter' => $request->getServer('OWO_DB_ADAPTER', install_get_current_db_driver()),
            'dbNext' => $request->getServer('OWO_DB_NEXT', 'none')
        ];
    } else {
        $config = $request->from([
            'dbHost',
    ...
    // init db structure
    try {
        $scripts = file_get_contents(__OWO_ROOT_DIR__ . '/install/' . $type . '.sql');
        $scripts = str_replace('OWO_', $config['dbPrefix'], $scripts);
    ...
    <div class="row OWO-page-main">
        <div class="col-mb-12 col-tb-8 col-tb-offset-2">
            ...
            <form autocomplete="off" action="install.php" method="post">
                <ul class="OWO-option">
                    <li>
                        <label class="OWO-label" for="userUrl"><?php _e('网站地址'); ?></label>
                ...
                <ul class="OWO-option">
                    <li>
                        <label class="OWO-label" for="userName"><?php _e('用户名'); ?></label>
                ...
                <ul class="OWO-option">
                    <li>
                        <label class="OWO-label" for="userPassword"><?php _e('登录密码'); ?></label>
                ...
                <ul class="OWO-option">
                    <li>
                        <label class="OWO-label" for="userMail"><?php _e('邮件地址'); ?></label>
...
function install_step_3_perform()
{
    ...
    if (install_is_cli()) {
        $config = [
            'userUrl' => $request->getServer('OWO_SITE_URL'),
            'userName' => $request->getServer('OWO_USER_NAME', 'OWO'),
            'userPassword' => $request->getServer('OWO_USER_PASSWORD'),
            'userMail' => $request->getServer('OWO_USER_MAIL', 'admin@localhost.local')
        ];
    } else {
        $config = $request->from([
            'userUrl',
    ...
    try {
        ...
        $installDb->query(
            $installDb->insert('table.contents')->rows([
                'title' => _t('欢迎使用 OWO'),
                'slug' => 'start', 'created' => \OWO\Date::time(),
                'modified' => \OWO\Date::time(),
        ...
        $installDb->query(
            $installDb->insert('table.contents')->rows([
                ...
                'created' => \OWO\Date::time(),
                'modified' => \OWO\Date::time(),
                'text' => '<!--markdown-->' . _t('本页面由 OWO 创建, 这只是个测试页面.'),
                'authorId' => 1,
                'order' => 0,
                'type' => 'page',
                'status' => 'publish',
...
function install_dispatch()
{
    ...
    if (install_is_cli()) {
        define('__OWO_ROOT_URL__', 'http://localhost');
    }
