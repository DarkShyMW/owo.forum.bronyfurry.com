...
$actionUrl = $security->getTokenUrl(
    \owo\Router::url('do', array('action' => 'backup', 'widget' => 'Backup'),
        \owo\Common::url('index.php', $options->rootUrl)));
        ...
        <div class="row owo-page-main" role="main">
            ...
            <div id="backup-secondary" class="col-mb-12 col-tb-4" role="form">
                ...
                <form action="<?php echo $actionUrl; ?>" id="from-server" class="tab-content hidden" method="post">
                    ...
                    <ul class="owo-option">
                        <li>
                            <p class="description"><?php _e('将备份文件手动上传至服务器的 %s 目录下后, 这里会出现文件选项', __owo_BACKUP_DIR__); ?></p>
                        </li>
                    </ul>
                    <?php else: ?>
                    <ul class="owo-option">
                        <li>
                            <label class="owo-label" for="backup-select-file"><?php _e('选择一个备份文件恢复数据'); ?></label>
...
?>
<script>
    $('#backup-secondary .owo-option-tabs li').click(function() {
        $('#backup-secondary .owo-option-tabs li').removeClass('active');
