...
$isAllComments = ('on' == $request->get('__OWO_all_comments') || 'on' == \OWO\Cookie::get('__OWO_all_comments'));
?>
<div class="main">
    <div class="body container">
        <?php include 'page-title.php'; ?>
        <div class="row OWO-page-main" role="main">
            <div class="col-mb-12 OWO-list">
                <div class="clearfix">
                    <ul class="OWO-option-tabs right">
                    <?php if($user->pass('editor', true) && !isset($request->cid)): ?>
                        <li class="<?php if($isAllComments): ?> current<?php endif; ?>"><a href="<?php echo $request->makeUriByRequest('__OWO_all_comments=on'); ?>"><?php _e('所有'); ?></a></li>
                        <li class="<?php if(!$isAllComments): ?> current<?php endif; ?>"><a href="<?php echo $request->makeUriByRequest('__OWO_all_comments=off'); ?>"><?php _e('我的'); ?></a></li>
                    <?php endif; ?>
                    </ul>
                    <ul class="OWO-option-tabs">
                ...
                <div class="OWO-list-operate clearfix">
                    <form method="get">
                        <div class="operate">
                            <label><i class="sr-only"><?php _e('全选'); ?></i><input type="checkbox" class="OWO-table-select-all" /></label>
                ...
                </div><!-- end .OWO-list-operate -->
                
                <form method="post" name="manage_comments" class="operate-form">
                <div class="OWO-table-wrap">
                    <table class="OWO-list-table">
                        <colgroup>
                        ...
                        <?php else: ?>
                        <tr>
                            <td colspan="4"><h6 class="OWO-list-table-title"><?php _e('没有评论') ?></h6></td>
                        </tr>
                        <?php endif; ?>
                        </tbody>
                    </table><!-- end .OWO-list-table -->
                </div><!-- end .OWO-table-wrap -->
...
$(document).ready(function () {
    ...
    function rememberScroll () {
        $(window).bind('beforeunload', function () {
            $.cookie('__OWO_comments_scroll', $('body').scrollTop());
        });
    }

    // 自动滚动
    (function () {
        var scroll = $.cookie('__OWO_comments_scroll');

        if (scroll) {
            $.cookie('__OWO_comments_scroll', null);
